...
<main>
  ...
  <div class="controls">
    ...
    <label>
      Speed
      <input type="range" min="0.1" max="50" step="0.1" bind:value={speed} />
      <span>{speed.toFixed(1)}d/f</span>
    </label>
  ...
  <div class="solar-system">
    <svg viewBox="0 0 {viewSize} {viewSize}" xmlns="http://www.w3.org/2000/svg">
      ...
      {#each zodiacSigns as sign, i}
        ...
        <!-- sector line -->
        <line
          x1={cx + (zodiacRadius - 15) * Math.cos(startAngle)}
          y1={cy + (zodiacRadius - 15) * Math.sin(startAngle)}
        ...
        <!-- symbol -->
        <text
          x={cx + (zodiacRadius - 8) * Math.cos(midAngle)}
          y={cy + (zodiacRadius - 8) * Math.sin(midAngle)}
          text-anchor="middle"
